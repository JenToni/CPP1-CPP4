...
void GLWidget::receiveCommand(s21::Command &cmd) {
  cmd_ = &cmd;
  firstiInitialize = cmd.getFirstInit();
  if (vertices != cmd_->getVertices() || firstiInitialize) {
...
void GLWidget::mouseMoveEvent(QMouseEvent *event) {
  if (this->rotationInitializied) {
    ...
    cmd_->setRotationX(angle_x, true);
    cmd_->setRotationY(angle_y, true);
    this->rotationInitializied = 0;
  }
  cmd_->setRotationX(
      fixed_angle_x + 1 / M_PI * (event->pos().y() - mousePosition.y()),
      true);  // вывести на экран!
  cmd_->setRotationY(
      fixed_angle_y + 1 / M_PI * (event->pos().x() - mousePosition.x()), true);
}

void GLWidget::wheelEvent(QWheelEvent *event) {
  if (event->angleDelta().y() > 0 && scale < 10) {
    cmd_->setScale(scale + 0.25f, 1., true);
