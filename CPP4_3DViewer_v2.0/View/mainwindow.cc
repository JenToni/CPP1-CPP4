...
GifRecorder::GifRecorder(GLWidget *gl, int GifMode) {
  glInterface = gl;
  frameCount = 0;
  gifMode = GifMode;
  tmr = new QTimer();
...
void MainWindow::on_pushButton_Gif_mode_2_clicked() {
  gifRecorder = new GifRecorder(glInterface, 2);
  gifRecorder->RecordGif();
}
